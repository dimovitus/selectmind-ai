...
1. Global hotkey `Ctrl+Shift+X` or tray → **Capture region**
2. Main window hides; transparent **`capture-overlay`** covers the monitor under the cursor
3. User selects a region; OS capture via **xcap** (Windows DXGI / Desktop Duplication)
4. OCR: **Windows.Media.Ocr** (Auto/Windows) with **Tesseract.js** fallback
...
| 8 | Settings → OCR **Auto** | Windows OCR used when available |
...
- **Protected content** (DRM video) — capture may be black by design.
...
| API keys | Windows Credential Manager (Settings → Security note) |
